...
class TwoImageIterator(Iterator):
    ...
    def __init__(self, directory, a_dir_name='A', b_dir_name='B',OD_dir_name='OD', load_to_memory=False,
                 is_a_binary=False,is_OD_binary=False, is_b_binary=False, is_a_grayscale=False,is_OD_grayscale=False,
        ...
        self.OD_dir = os.path.join(directory, OD_dir_name)

        a_files = set(x for x in os.listdir(self.a_dir))
        b_files = set(x for x in os.listdir(self.b_dir))
        OD_files = set(x for x in os.listdir(self.OD_dir))
        # self.filenames = list(a_files.intersection(b_files))
        self.filenames = list(a_files.intersection(b_files.intersection(OD_files)))
            
        # print("self.filenames............. ALI "+ str(self.filenames))
        # Use only a subset of the files. Good to easily overfit the model
        if N > 0:
            # print("INSIDE checking dataset size............. ALI" + str(N))
    ...
    def _load_imgs_to_memory(self):
        ...
        self.OD = np.zeros((self.N,) + self.image_shape_OD)

        for idx in range(self.N):
            ai, bi ,ODi= self._load_img_pair(idx, False)
            self.a[idx] = ai
            self.b[idx] = bi
            self.OD[idx] = ODi
    ...
    def _load_img_pair(self, idx, load_from_memory):
        ...
        if load_from_memory:
            ...
            b = load_img(os.path.join(self.b_dir, fname),
                         ...
                         target_size=self.target_size)
            OD = load_img(os.path.join(self.OD_dir, fname),
                         grayscale=self.is_OD_grayscale,
                         target_size=self.target_size)

            a = img_to_array(a, self.dim_ordering)
            b = img_to_array(b, self.dim_ordering)
            OD = img_to_array(OD, self.dim_ordering)

        return a, b, OD

    def _random_transform(self, a, b, OD):
        """
        ...
        b = apply_transform(b, transform_matrix, img_channel_index,
                            fill_mode=self.fill_mode, cval=self.cval)
        OD = apply_transform(OD, transform_matrix, img_channel_index,
                            fill_mode=self.fill_mode, cval=self.cval)
        ...
        if self.horizontal_flip:
            if np.random.random() < 0.5:
                ...
                OD = flip_axis(OD, img_col_index)

        if self.vertical_flip:
            if np.random.random() < 0.5:
                a = flip_axis(a, img_row_index)
                b = flip_axis(b, img_row_index)
                OD = flip_axis(OD, img_row_index)
    ...
    def next(self):
        ...
        for i, j in enumerate(index_array):
            a_img, b_img, OD_img = self._load_img_pair(j, self.load_to_memory)
            a_img, b_img, OD_img = self._random_transform(a_img, b_img, OD_img)
        ...
        if self.is_OD_binary:
            batch_OD = self._binarize(batch_OD)
        else:
            batch_OD = self._normalize_for_tanh(batch_OD)
            
        return [batch_a, batch_b, batch_OD]
